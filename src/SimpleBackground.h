...
private:
    ...
    cv::cuda::GpuMat d_frame, d_frameGray, d_frameComparison;
    std::vector<cv::cuda::GpuMat> d_frameHistory;
    ...
    cv::cuda::GpuMat diffResultBinary, d_magMask, d_magnitudeNorm, d_diffModified;
    cv::cuda::GpuMat d_diff, d_resultBasicDiff, d_result;
    cv::cuda::GpuMat d_flowFarneback, d_magnitude, d_flowXY[2], d_map, d_mapXY[2];

    cv::Mat map, flow, magnitude, frameGray, frameComparison;
