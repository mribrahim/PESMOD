...
namespace gSLICr
{
	namespace engines
	{
		...
		public:
			...
			// added on 12.2020 to draw edges only
            void Draw_Segmentation_Edges(UChar4Image* out_img);

			// Write the segmentation result to a PGM image
			void Write_Seg_Res_To_PGM(const char* fileName, unsigned short *lable_array);
		};
	}
}
